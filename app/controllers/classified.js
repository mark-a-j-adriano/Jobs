app.controller('classifiedCTRL', function ($sce, $state, $auth, $uibModal, $stateParams, $timeout, toastr, focus, Upload, DataFactory, StorageFactory, currentUser) {
    //console.log('START - classifiedCTRL');
    ...
    */
    ...
    vm.getTmpID = function () {
        DataFactory.getChildRequestorInf($stateParams.orderID).then(
            ...
            function (response) {
                ...
                vm.task.creative_form = 'classified';
                vm.task.status = 'new';
    ...
    vm.getTask = function () {
        DataFactory.getTask($stateParams.taskID, 'classified').then(
            //success
            function (response) {
    ...
    vm.saveDraftTask = function () {
        ...
        DataFactory.uploadTask(vm.task, 'classified', passedID).then(
            //success
            function (response) {
                    ...
                    resolve: {
                        parentData: function () {
                            var tmp = {
                                ...
                                frm_title: 'Success',
                                isConfirm: false,
                                msg: "Successfully saved ClassifiedLAB task and you will be redirected to dashboard.",
                            };
                            return tmp;
                        },
                        msgList: function () {
    ...
    vm.submitTask = function (newStatus) {
        ...
        if (vm.isValid) {
            ...
            DataFactory.uploadTask(tmpTsk, 'classified', passedID).then(
                //success
                function (response) {
    ...
    vm.addProduct = function () {
        ...
        var modalInstance = $uibModal.open({
            ...
            controller: 'classifiedModalCtrl as ctrl',
            resolve: {
                parentData: function () {
                    var tmp = {
                        isList: true,
    ...
    vm.requireFileName = function () {
        var res = angular.copy(vm.task.artwork_type);
        res = res.toLowerCase().trim();
        if (res.includes("amendment")) {
            vm.isFldDisabled = false;
        } else if (res.includes("re-import")) {
            vm.isFldDisabled = false;
        } else {
            vm.isFldDisabled = true;
        }
    ...
    vm.closeClassifiedTask = function () {
        //  completed
...
app.controller('classifiedModalCtrl', function ($uibModalInstance, focus, toastr, parentData, product) {
    var vm = this;
    ...
    vm.clrField = function (fld) {
        //console.log("[clrField] fld : " + fld);
        if (fld.includes("etNum")) {
            var flag = false;
            if (fld == 'etNum') {
